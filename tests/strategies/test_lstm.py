"""``LSTMStrategy``: warmup, decision mapping, and end-to-end causality."""
...
def saved_model(tmp_path_factory):
    """Train a tiny model on synthetic bars and register it; return (dir, id)."""
    ...
    spec = WindowSpec(
        window=WINDOW, feature_set="price_v1", base_horizon=5, context_horizons=(), label=label
    )
...
def test_warmup_is_window_plus_feature_warmup(saved_model):
    ...
    strat = get_strategy("lstm")(model=model_id, models_dir=root)
...
def test_holds_until_warmed_up_then_decides(saved_model, make_context):
    root, model_id = saved_model
    strat = get_strategy("lstm")(model=model_id, models_dir=root, threshold=0.0)
...
def test_on_no_signal_is_configurable(saved_model, make_context):
    ...
    # a threshold of ~1 can never be cleared -> always the no-signal branch
    hold_strat = get_strategy("lstm")(model=model_id, models_dir=root, threshold=0.99)
    flat_strat = get_strategy("lstm")(
...
def test_bracket_carries_the_trained_barriers(saved_model, make_context):
    root, model_id = saved_model
    strat = get_strategy("lstm")(model=model_id, models_dir=root, threshold=0.0)
    bars = _bars(strat.warmup + 80, seed=2)

    decisions = [
        strat.on_bar(make_context(bars.iloc[:i])) for i in range(strat.warmup + 1, len(bars))
    ]
    targets = [d for d in decisions if isinstance(d, Target)]
...
def test_no_bracket_when_disabled(saved_model, make_context):
    root, model_id = saved_model
    strat = get_strategy("lstm")(model=model_id, models_dir=root, threshold=0.0, bracket=False)
    bars = _bars(strat.warmup + 80, seed=2)
    targets = [
        d
    ...
    assert targets
    assert all(t.max_bars is None and t.stop is None for t in targets)
...
def test_requires_torch(monkeypatch, saved_model):
    import builtins
    ...
    with pytest.raises(ValueError, match=r"\[model\]"):
        get_strategy("lstm")(model=model_id, models_dir=root)
